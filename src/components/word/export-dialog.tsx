...
export function ExportDialog({ content, request, onClose }: ExportDialogProps) {
  ...
  return (
    <Dialog open={!!content} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="flex max-h-[90dvh] gap-0 overflow-hidden p-0 sm:max-w-6xl">
        <div className="flex min-h-0 w-full flex-col">
          <DialogHeader className="border-b px-6 py-5">
            <div className="flex items-start justify-between gap-4 pr-8">
              ...
              <div className="hidden rounded-full border bg-muted/50 px-3 py-1 text-xs text-muted-foreground md:block">
                Word .docx
              </div>
            </div>
          </DialogHeader>
...
function DraftOption({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "flex w-full items-start gap-3 rounded-2xl border bg-background p-3 text-left transition-colors hover:bg-accent",
